 ...
 * Protótipo: usa placeholders elegantes. Trocar por fotos reais
 * dos casos (ex.: institutoareluna.pt/casos/...) na produção.
 */
        ...
        <div className="mt-14 grid gap-5 md:grid-cols-3">
          {p.galleryCaptions.map((cap, i) => (
            <Reveal
              key={cap}
              delay={i * 80}
              className="tick-card group relative aspect-[4/5] overflow-hidden rounded-sm border border-gold-leaf/20"
            >
              {/* placeholder gradiente — substituir por <Image> real */}
              <div className="absolute inset-0 bg-gradient-to-br from-jet-off via-[#33302b] to-jet" />
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="font-serif text-[5rem] font-light text-gold-leaf/15">
                  A
                </span>
              </div>
